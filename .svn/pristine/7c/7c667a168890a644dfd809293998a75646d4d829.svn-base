...
public class MybatisCriteriaPlugin extends PluginAdapter {
    ...
    public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(final IntrospectedTable introspectedTable) {
        ...
        addSetterComment(method, OutputUtilities.lineSeparator + "\t *            查询的条件名称" + OutputUtilities.lineSeparator + "\t * @param value" + OutputUtilities.lineSeparator
                + "\t *            查询的值", "condition");
        topLevelClass.addMethod(method);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (content.contains(introspectedTable.getExampleType())) {
                ...
                if (xml.getElements().size() == 3) {
                    ...
                    xml.getAttributes().get(0).setValue("Update_By_Example_Where_Clause");
                } else {
